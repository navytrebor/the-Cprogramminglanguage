...
while (next operator or operand is not end-of-file indicator)
	if (number)
		...
		error
```
### Comments on code in polishcalc.c:
...
A variable is external if it is defined outside of any function. Thus the stack and the stack index that must be shared by `push`anf `pop`are defined outside these functions. But `main`itself does not refer to the stack or stack position -  the representation can be hidden.
